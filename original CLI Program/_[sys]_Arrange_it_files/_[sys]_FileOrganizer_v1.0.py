...
def move_older_duplicates(duplicates, target_directory, report_file):
    ...
    with open(report_file, mode='w', newline='') as file:
        ...
        writer.writerow(['Original File', 'Duplicate File'])  # Write header
        ...
        for original_file, duplicate_file in duplicates:
            ...
            writer.writerow([original_file, duplicate_file])
...
def main_menu():
    ...
    while True:
        ...
        print("1. [Organize files] \n--> Skip folders and files that have '_[sys]_' in their name.\n--> If files exist with same name in same month, it will overwrite the previous moved file even if the content is different.\n")
        print("2. [Organize files] \n--> Only scan folders that have '_[scan]_' in their name.\n--> It will move only one file among files with same filename. \n--> only look in level 1 \n")
        ...
        if choice == '1':
            ...
            report_file = os.path.join(duplicate_dir, 'duplicate_report.csv')
            
            if duplicates:
                print(f"Found {len(duplicates)} duplicate file(s). Moving older duplicates...")
                move_older_duplicates(duplicates, duplicate_dir, report_file)
